...
import base64
import json
from TSGuardGNN import TDGNN, train_model, simulate_streaming, simulate_streaming_new
import torch
...
def main():
    ...
    if st.session_state.running:
        ...
        simulate_streaming(sensor_data_file, global_dashboard_placeholder, sliding_chart_placeholder, positions,graph_size, model_path)
        ...
        for idx, row in df.iterrows():
            ...
            # Prepare tensor
            inputs = torch.full((1, graph_size), float('nan'))
            for i, val in enumerate(svals):
                if val is not None:
                    inputs[0, i] = val
                    last_received_time[i] = current_time

            # === Run imputation if needed ===
            imputed_vals, last_received_time = simulate_streaming_new(
                model, edge_index, inputs, last_received_time, current_time
            )
            for j, val in imputed_vals.items():
                svals[j] = val
                sstates[j] = True

            # === Plotting logic (same as your existing code) ===
            ...
            print('Sliding data : ', sliding_data)
